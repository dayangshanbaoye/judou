...
Phase 3 当前先落地阅读闭环主路径，后续再补纠错和范围确认：

- `repo::get_reader_view`：一次读取书名、扁平 TOC、当前 TOC 面包屑、段落上下文与句子流；command 层不写 SQL。
- `repo::update_sentence_status`：只允许 `unread/read/understood/flagged` 四种状态，返回更新后的句子 DTO。
- `repo::merge_sentences` / `repo::split_sentence`：人工纠错只允许同段连续合并和合法 offset 拆分；操作后重排 `order_index`，保持段落文本拼接不变，并写入 `processing_log(source='manual')`。
- `repo::list_scope_nodes` / `repo::confirm_scope`：导入后展示目录范围；用户排除节点时删除其下 `sentences` 并写入 `processing_log(stage='classify', source='manual')`，守住“排除节点不参与断句”验收。
- `repo::list_processing_log` / `repo::promote_log_to_rule`：展示未解决台账，并从台账创建 `processing_rules` 后标记台账 resolved。
- `commands::get_reader_view` / `commands::update_sentence_status` / `commands::merge_sentences` / `commands::split_sentence` / `commands::get_scope_nodes` / `commands::confirm_scope` / `commands::list_processing_log` / `commands::promote_log_to_rule`：薄 IPC 层，打开本地 SQLite 后委托 repo。
- `src/api/reader.ts`：前端类型化 IPC 客户端，与 `06_IPC接口契约` 保持一致。
- `ImportPanel`：导入报告后展示范围确认列表，用户可勾选目录节点参与/排除断句。
- `ReaderPanel`：提供目录点击切换、面包屑、段落来源上下文、连续阅读/逐句精读/自测模式、句子状态可视、`understood` 标记、句子多选合并与单句 offset 拆分；阅读模式与纠错工具条固定在句子滚动区外，避免滚动后不可见。
- `ProcessingLogPanel`：显示当前书未解决处理台账，并提供“固化为规则”的最小入口。

当前 Phase 3 后续还可以继续打磨规则应用效果和模式交互，但主验收链路已具备进入 Phase 4 的条件。
