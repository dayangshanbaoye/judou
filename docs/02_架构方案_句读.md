...
- `commands::get_reader_view` / `commands::update_sentence_status` / `commands::merge_sentences` / `commands::split_sentence` / `commands::get_scope_nodes` / `commands::confirm_scope`：薄 IPC 层，打开本地 SQLite 后委托 repo。
- `src/api/reader.ts`：前端类型化 IPC 客户端，与 `06_IPC接口契约` 保持一致。
- `ImportPanel`：导入报告后展示范围确认列表，用户可勾选目录节点参与/排除断句。
- `ReaderPanel`：提供目录点击切换、面包屑、段落来源上下文、连续阅读/逐句精读/自测模式、句子状态可视、`understood` 标记、句子多选合并与单句 offset 拆分；拆分交互会先展开目标句子与说明，避免“设为拆分”这类开发型文案暴露给用户。

当前 Phase 3 后续还需要补规则固化入口，以及连续阅读/逐句精读模式的更完整交互。
