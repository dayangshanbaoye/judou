...
- `commands::get_reader_view` / `commands::update_sentence_status` / `commands::merge_sentences` / `commands::split_sentence`：薄 IPC 层，打开本地 SQLite 后委托 repo。
...
当前 Phase 3 后续还需要补范围确认页、规则固化入口，以及连续阅读/逐句精读模式的更完整交互。
