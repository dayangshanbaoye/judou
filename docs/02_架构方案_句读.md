...
- `commands::get_reader_view` / `commands::update_sentence_status`：薄 IPC 层，打开本地 SQLite 后委托 repo。
- `src/api/reader.ts`：前端类型化 IPC 客户端，与 `06_IPC接口契约` 保持一致。
- `ReaderPanel`：提供目录点击切换、面包屑、段落来源上下文、句子状态可视与 `understood` 标记。

当前 Phase 3 后续还需要补多句框选、合并/拆分纠错写台账、范围确认页，以及连续阅读/逐句精读模式的更完整交互。
